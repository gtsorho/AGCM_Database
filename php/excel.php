...
$sql = "SELECT `id`, `firstName`, `lastName`, `otherNames`, `dbirth`, `sex`, `programme`, `residence`, 
...
$end_date = strtotime("+60 days", $start_date); 
    while($start_date < $end_date){
        ...
        if ($thisday === $begining ){
                ...
                //set column headers
                $fields = array('id','First Name','Last Name', 'OtherNames', 'DoB', 'Sex','Programme', 'Residence', 'Department', 'Phone','Email','Start Date' );
                fputcsv($f, $fields, $delimiter);
                 
                //output each row of the data, format line as csv and write to file pointer
                while($row = $query->fetch_assoc()){
                    
                    $lineData = array($row['id'], $row['firstName'], $row['lastName'], $row['otherNames'], $row['dbirth'], $row['sex'], $row['programme'], 
